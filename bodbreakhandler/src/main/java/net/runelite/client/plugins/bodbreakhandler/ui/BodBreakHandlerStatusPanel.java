...
import static net.runelite.client.plugins.bodbreakhandler.BodBreakHandlerPlugin.CONFIG_GROUP;
...
@Slf4j
public class BodBreakHandlerStatusPanel extends JPanel
{
	private final ConfigManager configManager;
	private final BodBreakHandlerPlugin bodBreakHandlerPlugin;
	private final BodBreakHandler breakHandler;
	...
	BodBreakHandlerStatusPanel(BodBreakHandlerPlugin bodBreakHandlerPlugin, BodBreakHandler bodBreakHandler, Plugin plugin)
	{
		this.configManager = bodBreakHandlerPlugin.getConfigManager();
		this.bodBreakHandlerPlugin = bodBreakHandlerPlugin;
		this.breakHandler = bodBreakHandler;
		this.plugin = plugin;
		...
		if (bodBreakHandlerPlugin.disposables.containsKey(plugin))
		{
			Disposable seconds = bodBreakHandlerPlugin.disposables.get(plugin);
		...
		bodBreakHandlerPlugin.disposables.put(plugin, secondsDisposable);

		Disposable extraDataDisposable = bodBreakHandler
			.getExtraDataObservable()
			.subscribe((data) -> SwingUtil.syncExec(() -> this.extraData(data)));

		bodBreakHandlerPlugin.disposables.put(plugin, extraDataDisposable);
	...
	private void milliseconds(long ignored)
	{
		...
		Map<Plugin, Instant> startTimes = breakHandler.getStartTimes();

		if (startTimes.containsKey(plugin))
		{
			Duration duration = Duration.between(breakHandler.getStartTimes().get(plugin), now);
			runtimeLabel.setText(formatDuration(duration));
		}

		Map<Plugin, Integer> breaks = breakHandler.getAmountOfBreaks();

		if (breaks.containsKey(plugin))
		{
			breaksLabel.setText(String.valueOf(breakHandler.getAmountOfBreaks().get(plugin)));
		}

		if (!breakHandler.isBreakPlanned(plugin) && !breakHandler.isBreakActive(plugin))
		{
			timeLabel.setText("00:00:00");
			return;
		}

		if (breakHandler.isBreakPlanned(plugin))
		{
			Instant breaktime = breakHandler.getPlannedBreak(plugin);
		...
		else if (breakHandler.isBreakActive(plugin))
		{
			Instant breaktime = breakHandler.getActiveBreak(plugin);
		...
		boolean enabled = Boolean.parseBoolean(configManager.getConfiguration(BodBreakHandlerPlugin.CONFIG_GROUP, sanitizedName(plugin) + "-enabled"));

		if (enabled && breakHandler.getPlugins().get(plugin) && bodBreakHandlerPlugin.isValidBreak(plugin) && !breakHandler.isBreakPlanned(plugin) && !breakHandler.isBreakActive(plugin))
		{
			bodBreakHandlerPlugin.scheduleBreak(plugin);
		}
	}
	...
	private void onConfigChanged(ConfigChanged configChanged)
	{
		...
		{
			breakHandler.removePlannedBreak(plugin);
		}
	...
	private void statusPanel()
	{
		...
		if (!breakHandler.getPlugins().get(plugin) && !breakHandler.isBreakActive(plugin))
		{
			c.insets = new Insets(2, 0, 2, 0);
			c.weightx = 0;
			c.gridx = 0;
		...
		if (breakHandler.getPlugins().get(plugin))
		{
			boolean enabled = Boolean.parseBoolean(configManager.getConfiguration(BodBreakHandlerPlugin.CONFIG_GROUP, sanitizedName(plugin) + "-enabled"));
			...
			if (breakHandler.getPlugins().get(plugin) && bodBreakHandlerPlugin.isValidBreak(plugin) && !breakHandler.isBreakPlanned(plugin) && !breakHandler.isBreakActive(plugin))
			{
				bodBreakHandlerPlugin.scheduleBreak(plugin);
			}
		}
